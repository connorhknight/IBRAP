...
Marrow_A is a bone marrow sample from a set of 25. It contains 2994 cells and was sequenced using 10x Genomics and aligned using CellRanger. Please download the sample and load it into R using the following command: 

```{r, eval=F, echo=T}

library(IBRAP)

marrow_A <- readRDS(file.choose())
...
# NOTE: We have also included a function (perform.decontX) 
# to perform ambient RNA decontamination. However, you need to 
# install package: celda Please see the link 
# (https://github.com/campbio/celda) for installation instruction. 
# In most cases, you do not need to perform this function unless 
# your data suffers from ambient RNA contamination
...
DecontX creates a umap projection of samples and scales the colours according the amount of contamination presented within a cell. In this sample, we observe minimal contaminaton. WARNING: DecontX should only be used if you expect ambient RNA otherwise it can drastically affect downstream results. 
...
We can check the data out in out metadata folder by performing the following:

```{r, eval=T, echo=T}

marrow_A@sample_metadata
...
In some instances, certain genes require omission from the highly variable genes. This can be achieved with the following function: 

```{r, eval=F, echo=T}

# This function finds all ribosomal genes and removes them from the HVG list of the three assays
unwanted <- as.character(rownames(marrow_A)[grepl("^RPL" ,x=rownames(marrow_A))])
marrow_A <- remove.hvgs(object = marrow_A, assay = c('SCT', 'SCRAN', 'SCANPY'), 
...
```{r, eval=T, echo=T, out.width="200%", out.height="150%"}
