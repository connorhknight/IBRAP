...
plot.list[[10]] <- plot.reduced.dim(object = pancreas, reduction = 'seurat_umap', assay = 'SCT',
                                   clust.method = 'metadata', column = 'original.project', pt.size = 0.1) + 
  ggplot2::ggtitle('SCT_cca') + 
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5))

plot.list[[11]] <- plot.reduced.dim(object = pancreas, reduction = 'seurat_umap', assay = 'SCRAN',
                                    clust.method = 'metadata', column = 'original.project', pt.size = 0.1) + 
  ggplot2::ggtitle('SCRAN_cca') + 
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5))

plot.list[[12]] <- plot.reduced.dim(object = pancreas, reduction = 'seurat_umap', assay = 'SCANPY',
                                    clust.method = 'metadata', column = 'original.project', pt.size = 0.1) + 
...
knitr::include_graphics(path = 'integrated_labels.png')

```

As we can see, SCRAN with seurat integration led to a well defined separation of cell types. To confirm this we can produce feature plots to see how canonical markers were separated from eachother during the analysis. 
...
knitr::include_graphics(path = 'integrated_features.png')

```

From these we can gather that the analytical pipeline separated cell type by their canonical markers correctly. We can also confirm this with a differential expression analysis that should identify all the celltypes gene markers. 
...
SCT_DE <- perform.diffexp.all(object = pancreas, assay = 'SCRAN', 
                              test = 'MAST', 
