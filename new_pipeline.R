...
Read10X_output <- function(directory, 
  ...
  if(true.length != length(unique(genes_ensembl$V2))) {
    if(isTRUE(make.feat.unique)) {
      ...
      genes_ensembl$V2 <- make.unique(as.character(genes_ensembl$V2))
    }
...
results_integrated <- perform.seurat.diffexp(object = celseq_comb,
                                            assay = 'SCRAN',
                                            test = 'MAST', 
                                            latent.vars = c('original.project', 'RAW_total.counts'),
                                            identity = celseq_comb@methods$SCANPY@cluster_assignments$scanorama_Louvain$RNA_snn_res.0.2)
